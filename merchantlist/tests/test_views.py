...
class MerchantListPublishTests(TestCase):
    ...
    def test_merchant_list_recovers_from_stale_static_file(self) -> None:
        config = Config.objects.create(pk=7, merchant_version=4)
        Merchant.objects.create(domain="fresh.example")

        stale_payload = {
            "version": config.merchant_version - 1,
            "updated": "2000-01-01T00:00:00Z",
            "merchants": ["old.example"],
        }
        views.STATIC_FILE.parent.mkdir(parents=True, exist_ok=True)
        views.STATIC_FILE.write_text(json.dumps(stale_payload), encoding="utf-8")

        response = self.client.get(reverse("merchant_list"))
        self.assertEqual(response.status_code, 200)

        data = json.loads(response.content)
        expected_updated = timezone.localtime(
            config.updated_at, dt_timezone.utc
        ).isoformat().replace("+00:00", "Z")
        self.assertEqual(data["version"], config.merchant_version)
        self.assertEqual(data["updated"], expected_updated)
        self.assertEqual(data["merchants"], ["fresh.example"])

        stored = json.loads(views.STATIC_FILE.read_text(encoding="utf-8"))
        self.assertEqual(stored["version"], config.merchant_version)
        self.assertEqual(stored["merchants"], ["fresh.example"])
        self.assertEqual(stored["updated"], expected_updated)
