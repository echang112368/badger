...
def sanitize_json_fields(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        if "platforms" in existing_columns:
            cursor.execute(
                f"""
                UPDATE {table_name}
                SET platforms = ?
                WHERE platforms IS NOT NULL
                  AND JSON_VALID(platforms) = 0
                """,
                (json.dumps([]),),
            )
        if "primary_niches" in existing_columns:
            cursor.execute(
                ...
                UPDATE {table_name}
                SET primary_niches = ?
                WHERE primary_niches IS NOT NULL
                  AND JSON_VALID(primary_niches) = 0
                """,
                (json.dumps([]),),
            )
        if "audience_breakdown" in existing_columns:
            cursor.execute(
                f"""
                UPDATE {table_name}
                SET audience_breakdown = ?
                WHERE audience_breakdown IS NOT NULL
                  AND JSON_VALID(audience_breakdown) = 0
                """,
                (json.dumps({}),),
            )
