...
class OrchestratorCachingTests(TestCase):
    def setUp(self):
        ...
        cache.clear()
        self.business_context = {"filters": {"platform": "instagram", "niche": "beauty"}}
    ...
    @patch("creatorMatch.services.matching.orchestrator.score_candidates_with_ai_diagnostics")
    def test_force_refresh_bypasses_cache(self, mock_ai):
        mock_ai.return_value = (
            {1: {"score": 91, "reasoning": "Strong fit.", "creator_summary": "Great fit", "highlights": ["high engagement"]}},
            {"ai_attempted": True, "error_code": None, "error_message": None, "response_id": "resp_123"},
        )

        apply_matching_scores(list(self.cards), {"filters": {"platform": "instagram"}})
        apply_matching_scores(list(self.cards), {"filters": {"platform": "instagram"}}, force_refresh=True)
