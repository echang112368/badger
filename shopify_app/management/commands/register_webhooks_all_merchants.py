...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        customers_data_request_url = options["customers_data_request_url"]
        customers_redact_url = options["customers_redact_url"]
        shop_redact_url = options["shop_redact_url"]
        for merchant in MerchantMeta.objects.all():
            ...
            data_request_success = register_customers_data_request_webhook(
                store_domain, access_token, customers_data_request_url
            )
            customers_redact_success = register_customers_redact_webhook(
                store_domain, access_token, customers_redact_url
            )
            shop_redact_success = register_shop_redact_webhook(
                store_domain, access_token, shop_redact_url
            )
            if (
                success
                and uninstall_success
                and data_request_success
                and customers_redact_success
