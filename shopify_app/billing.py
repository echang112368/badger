...
def _parse_usage_line_item(line_items: Iterable[Dict[str, Any]]) -> Tuple[str, str, Optional[Decimal]]:
    """Return the first subscription line item ID for usage charges (if any).

    Shopify's Billing V2 no longer exposes plan details or fragments on
    ``AppSubscription`` line items, so we cannot introspect usage terms or
    capped amounts here. Instead, we treat the first returned line item as the
    metered component when building usage records and fall back to empty terms
    and capped amounts when the API does not return them.
    """
    ...
    for line_item in line_items or []:
        if isinstance(line_item, dict):
            ...
            if isinstance(plan, dict):
                typename = (plan.get("__typename") or "").lower()
                terms = plan.get("terms", "") or ""
                capped = plan.get("cappedAmount") or {}
                try:
...
def create_or_update_recurring_charge(
    ...
    *,
    ...
    Uses ``appSubscriptionCreateV2`` from the Admin GraphQL API.
