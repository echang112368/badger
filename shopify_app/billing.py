...
class ShopifyBillingConfig:
    """Configuration for creating recurring charges with Billing V2."""
    ...
    def from_settings(cls) -> "ShopifyBillingConfig":
        ...
        interval = getattr(settings, "SHOPIFY_BILLING_INTERVAL", cls.interval)
        currency_code = getattr(settings, "SHOPIFY_BILLING_CURRENCY", cls.currency_code)

        return cls(
            name=name,
            interval=str(interval or cls.interval),
            currency_code=str(currency_code or cls.currency_code).upper(),
        )
...
def create_or_update_recurring_charge(meta: MerchantMeta, *, return_url: str) -> dict:
    ...
    charge = {
        "id": "",
        "status": "PENDING" if confirmation_url else "",
...
def create_usage_charge(
    ...
    raise ShopifyBillingError(
        "Usage-based charges are not supported with Shopify Billing V2 subscriptions."
    )
