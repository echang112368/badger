...
def oauth_callback(request: HttpRequest):
    ...
    if meta:
        ...
        logger.info(
            "Attempting Shopify orders/create webhook registration for %s at %s.",
            normalised_shop,
            webhook_url,
        )
        try:
            ...
            if success:
                logger.info(
                    "Shopify orders/create webhook registered for %s.",
                    normalised_shop,
                )
            else:
                logger.warning(
                    "Shopify orders/create webhook registration returned no subscription for %s.",
                    normalised_shop,
                )
        except Exception:
            logger.exception(
