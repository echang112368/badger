...
"""PayPal endpoints default to the sandbox API but can be configured via env."""

PAYPAL_OAUTH_URL = os.environ.get(
    "PAYPAL_OAUTH_URL", "https://api-m.sandbox.paypal.com/v1/oauth2/token"
)
PAYPAL_PAYOUT_URL = os.environ.get(
...
def send_mass_payouts(ignore_date: bool = False) -> Dict[str, str]:
    ...
    for record in unpaid_entries:
        ...
        try:
            ...
            continue
        if not meta.refresh_onboarding_status(save=True)["onboarding_completed"]:
            continue
